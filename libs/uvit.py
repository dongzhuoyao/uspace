...
class Block(nn.Module):
    ...
    def forward(self, x, skip=None, _counter=None, timesteps=None, **kwargs):
        if self.use_checkpoint:
            result_x = torch.utils.checkpoint.checkpoint(self._forward, x, skip)
        else:
...
class UViT(nn.Module):
    def __init__(
        ...
        if self.num_classes > 0:
            ...
            self.extras = 2
            logging.warning(
                f"from UViT, Using label embedding with {self.num_classes} classes"
            )
        else:
            self.extras = 1
