...
from tools.utils_t2i import editing_attention_map_vit
...
class Attention(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        qkv = self.qkv(x)
        if ATTENTION_MODE == "math" or (
            "dissect_name" in kwargs
            and kwargs["dissect_name"] is not None
            and kwargs["dissect_name"]
            in ["p2p", "local_prompt", "sampled_image_editing"]
        ):
            ...
            attn = editing_attention_map_vit(attn, **kwargs)
            attn = self.attn_drop(attn)
            x = (attn @ v).transpose(1, 2).reshape(B, L, C)

            kwargs["_counter"]["block_id"] += 1
...
class Block(nn.Module):
    ...
    def forward(self, x, skip=None, **kwargs):
        if self.use_checkpoint:
            ...
            return self._forward(x, skip, **kwargs)

    def _forward(self, x, skip=None, **kwargs):
    ...
    def forward(
        ...
        **kwargs,
        ...
        for blk in self.in_blocks:
            x = blk(x, _counter=_counter, timesteps=timesteps, **kwargs)
            skips.append(x)
