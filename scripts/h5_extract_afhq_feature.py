...
from lfm_dataset.AFHQ import AFHQ
...
def main(resolution=256, debug=False, ds_name='AFHQ', subset='train', class_name='dog', _feat_h5_path='assets/datasets/AFHQ256_features.h5'):
    parser = argparse.ArgumentParser()

    assert class_name in ['cat', 'dog', 'wild']
    

    _feat_h5_path = _feat_h5_path.replace('_features.h5', f'_{class_name}_features.h5')
    print("save to ", _feat_h5_path)
    dataset = AFHQ(root="~/data_hhd/stargan-v2/data", size=resolution, subset=subset, class_name=class_name)
    ...
    if debug:
        _feat_h5_path = _feat_h5_path.replace(".h5", "debug.h5")

    _len = len(train_dataset_loader.dataset)
    print('create dataset, length: ', _len)

    f = h5py.File(_feat_h5_path, mode="w")
    f.close()

    f = h5py.File(_feat_h5_path, mode="a")
    f.create_dataset(
