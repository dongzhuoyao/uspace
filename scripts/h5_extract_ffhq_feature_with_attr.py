...
def main(
    ...
    debug=False,
    ds_name="ffhq",
    _cluster_h5_path="assets/datasets/ffhq256_features_with_attr11.h5",
    attr_dim=11,
):
    parser = argparse.ArgumentParser()

    dataset = FFHQ_From1024(root="~/data_hhd/ffhq/images1024x1024", size=256)
    ...
    f.create_dataset(
        "train_attr", data=np.ones(shape=(_len, attr_dim), dtype=np.float32) * -1
    )
    ...
    for batch in tqdm(train_dataset_loader, desc="{}/{}".format(idx, _len)):
        ...
        for moment, _attr in zip(moments, attrs):
            f["train_feat"][idx, :] = moment
            f["train_attr"][idx, :] = _attr
            idx += 1
